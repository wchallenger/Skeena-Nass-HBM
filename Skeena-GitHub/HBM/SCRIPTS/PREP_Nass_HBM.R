message("Preparing Nass analysis data")



# Data Prep ---------------------------------------------------------------
smax.dat <- smax.tab %>% filter(Basin == "Nass") %>% filter(Stock != "Bowser") %>% mutate(ID = seq_along(Stock))
sr.dat <- sr.tab %>% 
  filter(Basin == "Nass" & Stock %in% smax.dat$Stock) %>% 
  left_join(
    x = .,
