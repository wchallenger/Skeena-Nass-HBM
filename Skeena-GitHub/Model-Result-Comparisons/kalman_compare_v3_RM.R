...
for (stock in stocks) {
  ...
  HBM.yearly <- rbind(HBM.yearly, temp)
  rm(temp)
}
...
for (parm in c("ln.alpha",  "ln.alpha.c"))  {
  ...
  for (stock in stocks) {
    ...
    if (nrow(dat1) == 0) {
      message("No HBM estimates, skipping")
      next
    } 
    ...
    if (nrow(dat2) == 0) {
      message("No single stock estimates, skipping")
      next
    } 
    ...
    out.file <- file.path("Kalman", "by-stock", paste0("comparison--", parm, "-", str_replace(stock, "/","-"), ".pdf"))
    pdf(file = out.file, width = 15, height = 15)
    # par(mar = c(10, 4, 4, 2) + 0.1, mfrow = c(4,4))
    par(mar = c(10, 4, 4, 2) + 0.1, mfrow = c(2,2))
    models <- dat %>% count(Model) %>% select(Model) %>% unlist
    models <- c("HBM (m23)", setdiff(models, "HBM (m23)"))
    y.range <- dat %>% select(Min, Max) %>% gather(Type, Value) %>% filter(is.finite(Value)) %>% select(Value) %>% unlist %>% range
    for (m in models) {
      x <- dat %>% filter(Model == m) #%>% filter(Year < 1980)
